...
class CryptoCFB:
    ...
    iv : bytes
        ...
        It must be 16 bytes.
    bits : int, optional
        CFB mode, supported CFB modes : 8/64/128-bit CFB mode.
        (default is `128`)

    Attributes
    ----------
    next_vector : bytes
        The next vector for encryption or decryption.
    ...
    Except for the last part, every part must be multiple of (bits // 8) bytes.
    It can do encryption and decryption inplace to reduce memory footprint.
    """

    def __init__(self, key, iv, bits=128):
    ...
    def crypt_inplace(self, buf, is_encrypt=True):
        ...
        buf : bytearray
            ...
            If not the last part of the data, len(buf) must be multiple of (bits // 8).
